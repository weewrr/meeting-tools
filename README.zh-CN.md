...
**本地优先**的多人音视频会议与智能纪要工具。无需账号、无需公网服务器，会议数据完全保存在本机；局域网内的电脑 / 手机通过 HTTPS 即可免跨域接入，使用麦克风、摄像头、屏幕共享与实时字幕。
...
- 本工具仅供个人学习与研究使用，按"现状"提供，不作任何明示或默示的保证，详见 [LICENSE](LICENSE)。
- 转写字幕与 AI 智能纪要由 AI 模型生成，可能存在错误，**请务必在对外使用前人工校对纪要与会话记录**。
- 会议数据存储在本机，仅在局域网内共享；请勿在未做适当防护（如受信任的自签名证书、受控的访问密钥）的环境中讨论受法律保护的敏感内容。
- 录制会议时，用户需遵守相关法律法规并尊重所有参会者的权利，必要时取得同意。
...
- [LiveKit](https://livekit.io/) — SFU 媒体服务器
- [Vue.js](https://vuejs.org/) · [Element Plus](https://element-plus.org/) · [Vite](https://vitejs.dev/) — 前端框架
- [Spring Boot](https://spring.io/projects/spring-boot) — 后端框架
- [whisper.cpp](https://github.com/ggerganov/whisper.cpp) — 本地转写引擎
